...
import edu.elte.ik.preszk.cardgame.gamblingGames.BlackJackClient;
import edu.elte.ik.preszk.cardgame.gamblingGames.PokerClient;
...
public class App {
	...
	public static void main(String[] args) throws IOException {
		...
		while (valasztas != 0 && valasztas != 1 && valasztas != 2) { // eleg
																		// gagyi
		...
		switch (valasztas) {
		case 1:
			BufferedReader stdinSzerencse = new BufferedReader(new InputStreamReader(System.in));
			BlackJackClient blackJackClient = new BlackJackClient();
			PokerClient pokerClient = new PokerClient();
			SzerencseJatek szerencseJ = new SzerencseJatek(stdinSzerencse,blackJackClient,pokerClient);
			szerencseJ.printMenu();
			break;
		case 2:
